...
NC_PATH, GEOJSON_PATH = "./data/inten_phsc_2021.nc", "./data/prefectures.geojson"
...
LANGS = {
    "ja": {
        ...
        "no_map_data_warning": "{pref_name} には表示可能な水田データがありません。",
    },
    "en": {
        "title": "🌾 Simplified Rice Paddy CH₄ Emission Simulation",
        "subtitle": (
            "Estimating Potential Reduction Effects from Extended Mid‑season Drainage"
        ),
...
if st.session_state.sel_area:
    ...
    st.sidebar.info(t("no_selection"))
...
with st.spinner(t("loading_map_text")):
    ...
    boxes = [
        box(r.lon - dlon / 2, r.lat - dlat / 2, r.lon + dlon / 2, r.lat + dlat / 2)
    ...
    if gdf_clip.empty:
        ...
        if (
            ...
            or st.session_state.get("_prev_pref_for_map_init") != current_pref_for_map
        ):
